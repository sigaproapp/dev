(function () {
    ...
    function updateTrainingNavigation() { const indicator = document.getElementById('nav-indicator'); const buttons = [...document.querySelectorAll('#mobile-navigation > button')]; const trainingButton = document.getElementById('nav-training'); if (!indicator || !trainingButton || !buttons.length) return; const index = buttons.indexOf(trainingButton); const width = 100 / buttons.length; indicator.style.width = `${width}%`; indicator.style.left = `${index * width}%`; buttons.forEach(button => button.classList.toggle('bg-indigo-50', button === trainingButton)); }
    function openPage(pageId) { if (globalShowPage) globalShowPage(pageId === 'training-page' ? 'training' : 'dashboard'); document.querySelectorAll('main, section[id$="-page"]').forEach(element => element.classList.add('hidden')); document.getElementById(pageId)?.classList.remove('hidden'); if (pageId === 'training-page') updateTrainingNavigation(); }
    ...
    function buildShell() {
        ...
        button.addEventListener('click', () => { document.getElementById('fab-button')?.classList.add('hidden'); openPage('training-page'); renderList(); renderTrainingRanking('day'); });
            document.getElementById('app-shell').insertAdjacentHTML('beforeend', `<section id="training-page" class="hidden mx-auto w-[min(100%-2rem,64rem)] space-y-6 p-4"><div><h2 class="text-2xl font-black text-slate-800">Treinamentos</h2><p class="mt-1 text-xs font-medium text-slate-500">Conclua um conteúdo de cada treinamento para liberar a enquete.</p></div><div id="training-list" class="space-y-4"></div><section id="training-player" class="hidden rounded-[2rem] border border-slate-100 bg-white p-5 shadow-sm"><div class="flex items-start justify-between gap-3"><div><p id="training-status" class="text-[10px] font-black uppercase tracking-wider text-rose-600"></p><h3 id="training-title-view" class="mt-1 text-xl font-black text-slate-800"></h3></div><button id="training-close" type="button" class="flex h-9 w-9 items-center justify-center rounded-xl bg-slate-100 text-slate-500" aria-label="Fechar"><i data-lucide="x" class="h-4 w-4"></i></button></div><div id="training-video" class="mt-5 overflow-hidden rounded-2xl bg-slate-950"></div><div id="training-audio" class="mt-3"></div><details id="training-pdf" class="mt-3 hidden rounded-2xl border border-slate-100 bg-slate-50"><summary class="cursor-pointer px-4 py-3 text-xs font-black text-slate-700">Material em PDF</summary><div class="flex flex-wrap items-center gap-2 border-b border-slate-200 p-3"><button id="training-pdf-prev" type="button" class="rounded-lg bg-white px-3 py-2 text-xs font-black">Anterior</button><button id="training-pdf-next" type="button" class="rounded-lg bg-white px-3 py-2 text-xs font-black">Próxima</button><button id="training-pdf-zoom-out" type="button" class="rounded-lg bg-white px-3 py-2 text-xs font-black">-</button><span id="training-pdf-page" class="text-xs font-bold text-slate-500"></span><button id="training-pdf-zoom-in" type="button" class="rounded-lg bg-white px-3 py-2 text-xs font-black">+</button></div><div id="training-pdf-viewer" class="max-h-[70vh] overflow-auto bg-slate-200 p-3 text-center"><canvas id="training-pdf-canvas" class="mx-auto"></canvas></div></details><div id="training-quiz" class="mt-5"></div></section><section class="rounded-[2rem] border border-slate-100 bg-white p-5 shadow-sm"><div class="flex items-center justify-between"><h3 class="text-sm font-black uppercase tracking-wider text-slate-800">Ranking de acertos</h3><div class="flex rounded-xl bg-slate-100 p-1"><button data-ranking="day" class="training-period rounded-lg bg-white px-3 py-2 text-[10px] font-black text-rose-600">Hoje</button><button data-ranking="week" class="training-period rounded-lg bg-white px-3 py-2 text-[10px] font-black text-slate-500">Semana</button><button data-ranking="month" class="training-period rounded-lg bg-white px-3 py-2 text-[10px] font-black text-slate-500">Mês</button></div></div><div id="training-ranking" class="mt-6"></div></section></section>`);
    ...
    function renderRanking(period) {
        ...
        target.innerHTML = podium.length ? `<div class="flex items-end justify-center gap-3">${podium.map((row, index) => `<div class="flex w-1/3 flex-col items-center"><div class="mb-2 flex h-10 w-10 items-center justify-center rounded-full bg-rose-50 text-sm font-black text-rose-600">${index + 1}</div><p class="max-w-full truncate text-center text-[10px] font-black text-slate-600">${esc(row.name)}</p><div class="mt-2 flex h-${index === 0 ? '28' : index === 1 ? '20' : '16'} w-full items-center justify-center rounded-t-2xl ${colors[index]} text-xl font-black text-white">${row.score}</div></div>`).join('')}</div>` : '<p class="text-xs text-slate-400">Nenhum resultado ainda.</p>';
    }
    ...
    function renderTrainingRanking(period) {
        ...
        const start = new Date(); if (period === 'day') start.setHours(0, 0, 0, 0); else if (period === 'week') { start.setHours(0, 0, 0, 0); start.setDate(start.getDate() - (start.getDay() === 0 ? 6 : start.getDay() - 1)); } else { start.setDate(1); start.setHours(0, 0, 0, 0); }
        const ranking = users().filter(user => user.lojaId === session().lojaId && user.role === 'vendedor').map(user => { let total = 0; try { total = Object.values(JSON.parse(localStorage.getItem(`${KEY}-progresso::${user.id}`)) || {}).filter(item => item.completed && new Date(item.completedAt) >= start).reduce((sum, item) => sum + Number(item.score || 0), 0); } catch {} return { ...user, total }; }).sort((first, second) => second.total - first.total || String(first.name).localeCompare(String(second.name), 'pt-BR')).slice(0, 3);
        if (!ranking.length) { target.innerHTML = '<p class="w-full text-center text-xs font-semibold text-slate-400">Nenhum vendedor cadastrado.</p>'; return; }
        ...
        target.innerHTML = `<div style="display:grid;grid-template-columns:repeat(3,minmax(0,1fr));align-items:end;gap:0.75rem;width:100%">${podium.map(seller => { const position = ranking.indexOf(seller) + 1; const [height, barColor, borderColor, numberSize] = colors[position]; const avatarSize = position === 1 ? 'h-14 w-14' : 'h-10 w-10'; const crown = position === 1 ? '<i data-lucide="crown" class="absolute -top-4 left-1/2 h-5 w-5 -translate-x-1/2 text-yellow-500"></i>' : ''; return `<div style="min-width:0;display:flex;flex-direction:column;align-items:center"><div class="relative"><img src="https://ui-avatars.com/api/?name=${encodeURIComponent(seller.name)}&background=6366f1&color=fff" alt="${esc(seller.name)}" class="${avatarSize} mb-2 rounded-full border-2 ${borderColor} object-cover shadow-sm">${crown}</div><p class="mb-1 max-w-full truncate text-center text-[10px] font-black text-slate-600">${esc(seller.name)}</p><p class="mb-2 text-[10px] font-black text-rose-600">${seller.total} acertos</p><div class="${height} flex w-full items-center justify-center rounded-t-2xl ${barColor} font-black text-white ${numberSize}">${position}</div></div>`; }).join('')}</div>`; window.lucide?.createIcons();
    }

    function bindQuizInteractions() {
        const quiz = document.getElementById('training-quiz');
        if (!quiz || quiz.dataset.confirmationBound) return;
        quiz.dataset.confirmationBound = 'true';
        quiz.addEventListener('click', async event => {
            ...
            if (!state) return;
            if (event.target.closest('#training-release')) {
            ...
            if (choice && !state.answers[state.index]) {
                ...
                state.answers[state.index] = { selected: Number(choice.dataset.trainingChoice), correct: option.correct };
