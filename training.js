(function () {
    ...
    function enhanceTrainingForm() {
        ...
        values.insertAdjacentHTML('beforebegin', '<div class="w-full rounded-xl border border-slate-200 bg-slate-50/80 p-3"><div class="mx-auto w-full max-w-[20rem]"><p class="mb-2 text-[10px] font-black uppercase tracking-wider text-slate-400">Dias disponíveis</p><input id="training-calendar-month" type="month" value="'+today().slice(0, 7)+'" class="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-bold outline-none"><div id="training-calendar" class="mt-2 grid w-full grid-cols-7 gap-1.5"></div><label class="mt-2 flex items-center gap-2 text-[10px] font-bold text-slate-500"><input id="training-all-days" type="checkbox" checked class="h-4 w-4 rounded border-slate-300 text-rose-600"> Todos os dias</label><div id="training-dates" class="mt-2 flex flex-wrap gap-1"></div></div></div>');
        ...
        const renderCalendar = () => {
            ...
            calendar.innerHTML = labels.map(label => `<span class="flex h-5 items-center justify-center text-[9px] font-black text-slate-400">${label}</span>`).join('');
            for (let index = 0; index < firstDay.getDay(); index += 1) calendar.insertAdjacentHTML('beforeend', '<span class="h-9"></span>');
            for (let day = 1; day <= totalDays; day += 1) {
                const dateKey = `${month}-${String(day).padStart(2, '0')}`;
                const selected = dates.has(dateKey);
                calendar.insertAdjacentHTML('beforeend', `<button type="button" data-training-date="${dateKey}" class="h-9 w-full rounded-lg border px-0 text-[10px] font-black ${selected ? 'border-rose-500 bg-rose-500 text-white shadow-sm' : 'border-slate-200 bg-white text-slate-600 hover:border-rose-300'}">${day}</button>`);
            }
